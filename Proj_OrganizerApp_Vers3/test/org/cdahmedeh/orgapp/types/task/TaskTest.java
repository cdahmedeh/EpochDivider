...
public class TaskTest {
	@Test public void taskTitleShouldBeBlankStringIfTitleIsSetToNull(){
		assertEquals("", new Task(null).getTitle());
	}
	...
	@Test public void settingTaskTitleToNullShouldReallySetItToBlankString(){
		Task task = new Task("somestring");
		task.setTitle(null);
		assertEquals("", task.getTitle());
	}
	
	@Test public void taskHasCorrectDefaultMutabilitySettingWhenCreated(){
		assertEquals(TypeConstants.DEFAULT_MUTABILITY, new Task("some task").getMutability());
	}
	...
	@Test public void taskHasCorrectDefaultPermissibilitySettingWhenCreated(){
		assertEquals(TypeConstants.DEFAULT_PRIORITY, new Task("some task").getPriority());
	}
