...
public class PersistenceRun001ReadWriteFromDatabase {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					PersistenceRun001ReadWriteFromDatabase window = new PersistenceRun001ReadWriteFromDatabase();
	 ...
	 */
	public PersistenceRun001ReadWriteFromDatabase() {
		initialize();
	}
	...
	class JPaintFrameText extends JFrame {
		...
		public void paint(Graphics g) {
			...
			g.drawLine(30 + this.getWidth() - 30 * 2, 0,
					30 + this.getWidth() - 30 * 2, this.getHeight());
			g.drawLine(0, 50 + this.getHeight() - 50 * 2, this.getWidth(),
					50 + this.getHeight() - 50 * 2);

			drawString(g, longString, 30, 50, this.getWidth() - 30 * 2,
					this.getHeight() - 50 * 2);
		}
			...
			while (!text.isEmpty()) {
				...
				while (!text.isEmpty()) {
					...
					if (nextWordWidth + 5 >= width && lineLength == 0) {
						...
						g.drawString(
								drawCroppedString(fontMetrics, nextWord, width),
								xPrintPos + lineLength, yPrintPos);
						break;
					} else if (lineLength + nextWordWidth + 5 < width) {
						g.drawString(nextWord, xPrintPos + lineLength,
								yPrintPos);
					} else {
						text.push(nextWord);
				...
				if (yPrintPos - y > height)
					break;
			}
		}

		private String drawCroppedString(FontMetrics metrics, String nextWord,
				int width) {
			if (nextWord.isEmpty() || metrics.stringWidth(nextWord) < width) {
				return nextWord;
			} else {
				return drawCroppedString(metrics,
						nextWord.substring(0, nextWord.length() - 1), width);
			}
		}
	}
}
