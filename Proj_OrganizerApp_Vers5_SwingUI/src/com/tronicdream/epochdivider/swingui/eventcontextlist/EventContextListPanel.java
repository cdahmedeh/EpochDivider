...
import com.tronicdream.epochdivider.swingui.taskcontextlist.ContextTransferable;

public class EventContextListPanel extends CPanel {
	private static final long serialVersionUID = -8250528552031443184L;
	public EventContextListPanel(DataContainer dataContainer, EventBus eventBus) {super(dataContainer, eventBus, "Context List");}
	...
	protected Object getEventRecorder() {
		return new Object(){
			...
			@Subscribe public void eventListChanged(EventsChangedNotification notification){
				refreshContextListTreeTable();
			}
			@Subscribe public void selecteFirstContextWhenEventListFinishesLoading(EventListPanelPostInitCompleteNotification notification){
	...
	private EventContextListTableFormat contextListTableFormat;
	
	@Override
	protected void windowInit() {
		setPreferredSize(new Dimension(EventContextListPanelDefaults.DEFAULT_CONTEXT_PANEL_WIDTH, EventContextListPanelDefaults.DEFAULT_CONTEXT_PANEL_HEIGHT));
...
//		setBorder(UIManager.getBorder("ScrollPane.border"));
	...
	private void createToolbar() {
		...
		JButton addContextButton = ToolbarHelper.createToolbarButton(toolbar, "Add Context", EventContextListPanel.class.getResource("/com/tronicdream/epochdivider/imt/icons/add.gif"));
	...
	private void prepareContextListTableModelAndRenders() {
		...
		contextListTableFormat = new EventContextListTableFormat(dataContainer);
		contextEventList.addAll(dataContainer.getEventContexts());
		...
		TableColumn dueDateColumn = contextListTable.getColumnModel().getColumn(EventContextListPanelDefaults.COLUMN_CONTEXT_COLOR);
		dueDateColumn.setCellRenderer(new ColorHueCellRenderer());
		
		TableColumn progressColumn = contextListTable.getColumnModel().getColumn(EventContextListPanelDefaults.COLUMN_CONTEXT_PROGRESS);
	...
	private void adjustContextListTableColumnWidths() {
		contextListTable.getColumnModel().getColumn(EventContextListPanelDefaults.COLUMN_CONTEXT_COLOR).setMaxWidth(EventContextListPanelDefaults.COLUMN_CONTEXT_COLOR_MAX_WIDTH);
		contextListTable.getColumnModel().getColumn(EventContextListPanelDefaults.COLUMN_CONTEXT_PROGRESS).setMaxWidth(EventContextListPanelDefaults.COLUMN_CONTEXT_PROGRESS_MAX_WIDTH);
	}
	...
	private void notifyThatSelectedContextChanged() {
		...
		if (selectedContext != null) {
			dataContainer.setSelectedEventContext(selectedContext);
	...
	private void addNewContextToContextListTable() {
		...
		dataContainer.emEventContextNew();
		refreshContextListTreeTable();
		
		//Init. editing the title of the new context and focus the editor.
		int rowOfNewContext = contextListTable.getRowCount()-1;
		contextListTable.editCellAt(rowOfNewContext, EventContextListPanelDefaults.COLUMN_CONTEXT_NAME);
		...
		TableHelper.scrollTableToCellAt(contextListTable, rowOfNewContext, EventContextListPanelDefaults.COLUMN_CONTEXT_NAME);
	...
	class ContextListPanelTransferHandler extends TransferHandler {
		...
		public boolean canImport(TransferSupport support) {
			...
			if (support.getTransferable().isDataFlavorSupported(new DataFlavor(Context.class, "Context"))) {
				...
				return true;
			} else if (support.getTransferable().isDataFlavorSupported(new DataFlavor(Event.class, "Event"))) {
...
//				int row = ((JTable.DropLocation)support.getDropLocation()).getRow();
//				int newIndexForMovedContext = dataContainer.emEventContextMoveToIndex(context, row);
//
...
//				return true;
//			} else if (support.getTransferable().isDataFlavorSupported(new DataFlavor(Event.class, "Event"))) {
//				//Get the event that was dragged.
//				Event event = null;
//				try {
//					event = (Event) support.getTransferable().getTransferData(new DataFlavor(Event.class, "Event"));
//				} catch (UnsupportedFlavorException | IOException e) {
//					return false;
//				}
//				
...
//				}
//				
//				//Change the context of event that was dragged in.
//				boolean changed = dataContainer.emEventSetContext(event, context);
//				
